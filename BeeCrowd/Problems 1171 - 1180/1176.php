...
Entrada
A primeira linha da entrada contém um inteiro T, indicando o número de casos de teste. Cada 
caso de teste contém um único inteiro N (0 ≤ N ≤ 60), correspondente ao N-esimo termo da série 
de Fibonacci.
...
# Variavel que indica a quantidade de loops 
$testes = (int)readline('Testes: ');
function fibo($indexFibonacci)
{
    $fibonacci = array(0, 1, 1);
    while (count($fibonacci) - 1 < $indexFibonacci) {
        $fibonacci[] = $fibonacci[count($fibonacci) - 2] + $fibonacci[count($fibonacci) - 1];
    }
    echo "Fib($indexFibonacci) = " . $fibonacci[$indexFibonacci] . "\n";
}
for ($i = 0; $i < $testes; $i++) {
    $indexFibonacci = (int)readline();
    while ($indexFibonacci < 0) {
        echo 'Número invalido, insira um Inteiro positivo.' . "\n";
        $indexFibonacci = (int)readline();
    }
    fibo($indexFibonacci);
}
